...
def search_page(first_name, middle_name, last_name, city, state, row, driver, output, is_time_out):
    ...
    try:
        ...
        print("No result appear")
        pass
    profiles_data = []
    if result_blocks:
        for result_block in result_blocks:
            req_sk_full_name = sk_first_name = sk_middle_name = sk_last_name = sk_age = None
            try:
                profile_url = result_block.find_element(By.XPATH, 'div/div/a')
                profile_url = profile_url.get_attribute('href')
                sk_full_name = result_block.find_element(By.XPATH, 'div/div/h3')

                if sk_full_name:
                    if 'age' in sk_full_name.text.lower():
                        req_sk_full_name = sk_full_name.text.split('Age')[0].strip().replace(',', '')
                        sk_age = sk_full_name.text.split('Age')[-1].strip()
                    else:
                        req_sk_full_name = sk_full_name.text.strip().replace(',', '')
                        sk_age = None
                    ...
                    if len(temp_sk_full_name) > 2:
                        ...
                        sk_middle_name = temp_sk_full_name[1:-1]

                        if sk_middle_name:
                            sk_middle_name = ' '.join(sk_middle_name)
                ...
                if live_locations:
                    # live_locations = live_locations.find_element(By.XPATH, '..')
                    locations += [live_locations.text.replace('Lives in:', '').strip().replace(', ', ' ')]

                # extract used locations
                used_locations_element = result_block.find_element(By.XPATH,
                                                                   './/div[contains(text(), "Prior addresses:")]')
                if used_locations_element:
                    used_locations_text = used_locations_element.text.replace('Prior addresses:', '').strip().split(
                        ', ')

                    # Combine each pair of adjacent elements
                    locations += [f'{used_locations_text[i]} {used_locations_text[i + 1]}' for i in
                                  range(0, len(used_locations_text), 2)]
        ...
        try:
            ...
            for data in profiles_data:
                # full_name = data[0]
                first_name = data[0]
                middle_name = data[1] if len(data) > 2 else None  # Check if index 2 exists
                last_name = data[2]
                # age = data[3]
                locations = data[-2]
                profile_url = data[-1]
                ...
                if first_name.strip().lower() == first_name.strip().lower() and last_name.strip().lower() == last_name.strip().lower() and match_location(
                        locations):
            ...
            for row in reader:
                ...
                try:
                    ...
                    if First_Name and Last_Name:
                        ...
                        if full_state_name:
                            url = f'https://www.usphonebook.com/{First_Name.strip().lower()}-{middle_name.strip().lower()}-{Last_Name.strip().lower()}/{city.strip().lower()}/{state.strip().lower()}'
                        else:
                            url = f'https://www.usphonebook.com/{First_Name.strip().lower()}-{middle_name.strip().lower()}-{Last_Name.strip().lower()}/{full_state_name}'
                        try:
                            sleep(1)
                            driver.refresh()
                            sleep(2)
